...
/*
// Funkce pro mapování ASCII na ZX matici (volaná callbackem)
void ZX_I2C_Callback(u8 status, char ch) {
	if ((unsigned char)ch > 127) return;

	bool bRelease = (status == 0x03);
	...
	// Převod na velká písmena pro getKeyPosition
	if (code >= 'a' && code <= 'z') code -= 32;

	// Mapování surových ASCII kódů
	if ((code >= 'A' && code <= 'Z') || (code >= '0' && code <= '9')) {
		char s[2] = {code, 0};
		keypos = getKeyPosition(s);
	}
	else if (code == 0x20) keypos = getKeyPosition("Break Space");
	else if (code == 0x0A) keypos = getKeyPosition("Enter");
	else if (code == 0xA2) keypos = getKeyPosition("Symbol Shift");
	else if (code == 0xC1) keypos = getKeyPosition("Caps Shift");

	if (keypos != 255) {
		if (!bRelease) {
			// PRESS
			rowKey[getRow(keypos)] &= getPressMask(keypos);
			if (getRowAlt(keypos) != 255)
				rowKey[getRowAlt(keypos)] &= getPressMaskAlt(keypos);
		} else {
			// RELEASE
			rowKey[getRow(keypos)] |= getReleaseMask(keypos);
			if (getRowAlt(keypos) != 255)
				rowKey[getRowAlt(keypos)] |= getReleaseMaskAlt(keypos);
		}
	}
}
*/
...
void ZX_I2C_Callback(u8 status, char ch) {
	...
	// --- OPRAVA PRO CLOCKWORK ---
	// Ignorovat I2C raw kódy pro D-Pad (0xB4-0xB7) a ABXY (0x81-0x84)
	// Tyto hardwarové klávesy obsluhuje výhradně funkce OSD_Input() v main.c
	if (raw_code >= 0x81 && raw_code <= 0x84) return;
	if (raw_code >= 0xB4 && raw_code <= 0xB7) return;
	// ----------------------------

	// 1. ZPRACOVÁNÍ BACKSPACE (0x08) -> Caps Shift + 0
	// (Ponecháváme pro jistotu, kdyby klávesnice uměla poslat přímo kód 0x08)
	if (raw_code == 0x08) {
		int k_cs = getKeyPosition("Caps Shift");
		int k_0 = getKeyPosition("0");

		if (!bRelease) {
			rowKey[getRow(k_cs)] &= getPressMask(k_cs);
			rowKey[getRow(k_0)] &= getPressMask(k_0);
		} else {
			rowKey[getRow(k_cs)] |= getReleaseMask(k_cs);
			rowKey[getRow(k_0)] |= getReleaseMask(k_0);
		}
		return;
	}

	// 2. UNSHIFT LOGIC (Zpětný převod symbolů na Číslo + Caps Shift)
	// Tímto obejdeme to, že STM32 už znaky "uvařil".
	char mapped_num = 0;
	switch (raw_code) {
		case '!': mapped_num = '1'; break;
		case '@': mapped_num = '2'; break;
		case '#': mapped_num = '3'; break;
		case '$': mapped_num = '4'; break;
		case '%': mapped_num = '5'; break;
		case '^': mapped_num = '6'; break;
		case '&': mapped_num = '7'; break;
		case '*': mapped_num = '8'; break;
		case '(': mapped_num = '9'; break;
		case ')': mapped_num = '0'; break;
	}

	if (mapped_num != 0) {
		int k_cs = getKeyPosition("Caps Shift");
		char s[2] = {mapped_num, 0};
		int k_num = getKeyPosition(s);

		if (!bRelease) {
			// Stiskneme obě klávesy v matici
			rowKey[getRow(k_cs)] &= getPressMask(k_cs);
			rowKey[getRow(k_num)] &= getPressMask(k_num);
		} else {
			// Uvolníme obě klávesy
			rowKey[getRow(k_cs)] |= getReleaseMask(k_cs);
			rowKey[getRow(k_num)] |= getReleaseMask(k_num);
		}
		return;
	}

	if (raw_code == 0xA5)      keypos = getKeyPosition("Symbol Shift");
	else if (raw_code == 0xA2) keypos = getKeyPosition("Caps Shift");
	else if (raw_code == 0x0A) keypos = getKeyPosition("Enter");
	else if (raw_code == 0x20) keypos = getKeyPosition("Break Space");
	else {
		// 4. Zpracování standardních ASCII znaků
		#ifndef KEY_MASK
		#define KEY_MASK 0x7F
		#endif

		u8 code = raw_code & KEY_MASK;
