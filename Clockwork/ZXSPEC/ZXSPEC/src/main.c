...
//overclocking picopad
	...
	InitDirtyTiles();
	// AKTIVACE CALLBACKU: I2C driver
	KeySetRawCallback(ZX_I2C_Callback);
	currPath[0] = '/';
	currPath[1] = 0;
	strMapFile[0] = 0;
	fillKeyInfo();
			...
			if (isFileAtPos(nMenuPos)) {
				...
				if (nLenTp > 4) {
					if ((strFileName[nLenTp - 3] == 'T') && (strFileName[nLenTp - 2] == 'A') && (strFileName[nLenTp - 1] = 'P')) {
						...
						case Z80SNAP_BROKEN:
							DrawClearZx();
							DrawTextZx("File is broken", 48, (HEIGHT - FONTH) / 2 - FONTH, COL_RED_ZX);
							DispUpdateZx();
							WaitMs(2000);
							bRefresh = true;
							break;
						case Z80SNAP_BADHW:
							DrawClearZx();
							DrawTextZx("Bad HW specification in file", 48, (HEIGHT - FONTH) / 2 - FONTH, COL_RED_ZX);
							DispUpdateZx();
							WaitMs(2000);
							bRefresh = true;
							break;
						case Z80SNAP_128:
							DrawClearZx();
							DrawTextZx("Unsupported ZX128 file", 48, (HEIGHT - FONTH) / 2 - FONTH, COL_RED_ZX);
							DispUpdateZx();
							WaitMs(2000);
							bRefresh = true;
							break;
...
// Menu pro výběr velikosti obrazovky
bool MenuDisplay() {
	bool bExit = false;
	bool bRefresh = true;
	bool bSave = false;
	...
	while (bExit == false) {
		...
		switch (ch) {
			...
			case KEY_DOWN:
				...
				bRefresh = true;
				break;
			case KEY_A: // Potvrzení Enterem
			case KEY_X: // Nebo Back
				KeyFlush();
				bExit = true;
				break;
			default:
		...
		if (bRefresh) {
			// Aplikujeme změnu ihned, aby uživatel viděl efekt (pokud bychom měli náhled)
			// Ale zde jen textově:

			DrawClearZx();
			DrawText2Zx("Display Size", (WIDTH - 12 * 16) / 2, 24, COL_YELLOW_ZX);

			// Vykreslení aktuální volby
			if (newZoom) {
				// Zeleně aktivní volba
				DrawTextBgZx(" 1.25x (Full) ", (WIDTH - 14 * 8) / 2, 80, COL_BLACK_ZX, COL_GREEN_ZX);
				DrawTextZx("  1:1 (Sharp) ", (WIDTH - 14 * 8) / 2, 100, COL_WHITE_ZX);

				DrawTextZx("Stretch to fill screen.", (WIDTH - 23 * 8) / 2, 140, COL_GRAY_ZX);
				DrawTextZx("Pixels are uneven.", (WIDTH - 18 * 8) / 2, 155, COL_GRAY_ZX);
			} else {
				DrawTextZx(" 1.25x (Full) ", (WIDTH - 14 * 8) / 2, 80, COL_WHITE_ZX);
				DrawTextBgZx("  1:1 (Sharp) ", (WIDTH - 14 * 8) / 2, 100, COL_BLACK_ZX, COL_GREEN_ZX);

				DrawTextZx("Native resolution.", (WIDTH - 18 * 8) / 2, 140, COL_GRAY_ZX);
				DrawTextZx("Perfect sharp pixels.", (WIDTH - 21 * 8) / 2, 155, COL_GRAY_ZX);
			}

			DrawTextZx("Use UP/DOWN to change", (WIDTH - 24 * 8) / 2, HEIGHT - 20, COL_YELLOW_ZX);
	...
	// Uložíme vybranou hodnotu do globální proměnné
	bZoom125 = newZoom;

	// Vynutíme kompletní překreslení (invalidace cache), protože se změnily souřadnice
	for (int i = 0; i < 6912; i++) cached[i] = 255;

	// Vyčistíme obrazovku, aby zmizel starý rámeček
	...
	case KEY_X: {
		...
		while (bExit == false) {
			...
			if (bRefresh) {
				...
				DrawTextBgZx("Y-load", WIDTH - 6 * 8, 0, RGBTO8(0x77, 0xDD, 0x77),
				COL_BLACK_ZX); //green
				DrawTextBgZx("B-save", WIDTH - 6 * 8, HEIGHT - 8, RGBTO8(0xFF, 0x69, 0x61), COL_BLACK_ZX); //red
				DispUpdateZx();
				bRefresh = false;
			}
