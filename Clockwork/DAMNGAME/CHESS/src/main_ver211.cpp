...
const sMove OpenMovesTab[4] = {
	{ 35, 55, PAWN+WHITE+NOMOVING, EMPTY, MOVEFLAG_NORMAL, 0, 0 },		  // pawn from E2 to E4
	{ 34, 54, PAWN+WHITE+NOMOVING, EMPTY, MOVEFLAG_NORMAL, 0, 0 },		  // pawn from D2 to D4
	{ 22, 43, KNIGHT+WHITE+NOMOVING, EMPTY, MOVEFLAG_NORMAL, 0, 0 },	// knight from B1 to C3
...
void Open()
{
	...
	while (True)
	{
		...
		for (int i = 0; i < num_items; i++)
		{
			...
			if (i == 5)
			{
				if (DeepMax == 2)      strcat(buffer, " 2");
				else if (DeepMax == 3) strcat(buffer, " 3");
				else if (DeepMax == 4) strcat(buffer, " 4");
				else if (DeepMax == 5) strcat(buffer, " 5");
				else if (DeepMax == 6) strcat(buffer, " 6");
				else if (DeepMax == 7) strcat(buffer, " 7");
				else                   strcat(buffer, " 8");
			}
...
int main()
{
	...
	while (True)
	{
        ...
        // Logic to start a new game or an edited game
        if (BoardIsEdited)
        {
