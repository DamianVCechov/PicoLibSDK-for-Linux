...
*/
// DAA instruction by C (GCC inline assembler)
u32 DAA_C(u32 reg_flags, u32 reg_ax)
{
    u32 res_ax, res_flags;
    __asm__ volatile (
        "pushfl\n\t"
        "popl %%edx\n\t"
        "andl %[not_flags_mask], %%edx\n\t"
        "movl %[in_flags], %%ecx\n\t"
        "andl %[flags_mask], %%ecx\n\t"
        "orl %%ecx, %%edx\n\t"
        "pushl %%edx\n\t"
        ...
        "movl %%eax, %[out_ax]\n\t"
        "movl %%edx, %[out_flags]\n\t"
        : [out_ax] "=r" (res_ax), [out_flags] "=r" (res_flags)
        : [in_flags] "r" (reg_flags), [in_ax] "r" (reg_ax),
          [not_flags_mask] "i" (~I8086_FLAGALL), [flags_mask] "i" (I8086_FLAGALL)
        : "eax", "ecx", "edx", "cc"
    );
    return ((res_flags & 0xFFFF) << 16) | (res_ax & 0xFFFF);
}

// DAS instruction by C (GCC inline assembler)
u32 DAS_C(u32 reg_flags, u32 reg_ax)
{
    u32 res_ax, res_flags;
    __asm__ volatile (
...
// 8-bit C-addition (AL+AH+C) by C (GCC inline assembler)
u32 ADC8_C(u32 reg_flags, u32 reg_ax)
{
    u32 res_ax, res_flags;
    __asm__ volatile (
...
// 8-bit C-subtraction (AL-AH-C) by C (GCC inline assembler)
u32 SBB8_C(u32 reg_flags, u32 reg_ax)
{
    u32 res_ax, res_flags;
    __asm__ volatile (
...
u32 ADC_E(u32 reg_flags, u32 reg_ax)
{
	...
	if ((f0 & M6502_D) != 0)
	{
		...
		if (((res & 0x0f) > 9) || ((xor_val & B4) != 0)) res += 6;
...
u32 SBC_E(u32 reg_flags, u32 reg_ax)
{
	...
	u16 xor_val = a ^ val ^ res;	// XOR
#if 1 // set to 0 to use alternative method (works with the same result)
	f |= ((xor_val ^ (xor_val >> 1)) >> 1) & M6502_V; // overflow flag
#else
